...
export default function DeallocationDlg(props) {
  const { open, handleOpen, curAPR, totalAllocation, deallocationFee } = props;

  const { xSakeDecimals, xSakeSymbol, allocatedOfDividends } = useXSakeState();
  ...
  const calcTotalAlloc =
    formatUnits(allocatedOfDividends, xSakeDecimals) - parseFloat(amount) > 0
      ? formatUnits(allocatedOfDividends, xSakeDecimals) - parseFloat(amount)
      : 0;
      ...
      <DialogBody className="pr-2">
        <div className="flex w-full justify-between min-w-full md:min-w-[100px] p-1 border">
          <input
            ...
            min="0"
            max={formatUnits(allocatedOfDividends, xSakeDecimals)}
            value={amount}
          ...
          <button
            ...
            onClick={(e) => {
              setAmount(formatUnits(allocatedOfDividends, xSakeDecimals));
            }}
          >
            max
          </button>
          ...
          <div className="w-full flex flex-col">
            ...
            <div className="w-full flex flex-row justify-between">
              ...
              <Typography variant="small" className="mr-4 text-left ">
                {(
                  (calcTotalAlloc /
                    (parseFloat(totalAllocation) - parseFloat(amount))) *
                  100
                ).toFixed(2)}{" "}
                %
              </Typography>
